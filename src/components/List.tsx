...
function reorderGuesses(guesses: string[], graphicData: any, r_country: number, countries: { features: any[] }) {
...
export default function List({
  ...
  function turnToCountry(e: SyntheticEvent, idx: string) {
    ...
    var angle = { lng: (countries.features[c_id].bbox[0] + countries.features[c_id].bbox[2]) / 2, lat: (countries.features[c_id].bbox[1] + countries.features[c_id].bbox[3]) / 2};
    ...
    <div ref={ref} className={"word-list z-10 expand-list select-none cursor-pointer" + (isMobile ? " top-12 p-2 right-1 left-1" : " top-0.5 w-3/12 left-1/2 -translate-x-1/2")} style={!isMobile ? {width: '440px'} : {}}>
      ...
      {orderedGuesses.length > 0 && (
        ...
        <p className="text-center text-gray-300">
          ...
          <span className="text-base"><FormattedMessage id="SortBy" /> </span>

          <button
          onClick={(e) => {e.stopPropagation(); setIsSortedByDistance(false);}}
          className="mt-2 transp">
            <span className={"text-base pointer " + (!isSortedByDistance ? "underline" : "")}>
              <FormattedMessage id="SortByGuesses"/>
            </span>
          </button>

          <button
          onClick={(e) => {e.stopPropagation(); setIsSortedByDistance(true);}}
